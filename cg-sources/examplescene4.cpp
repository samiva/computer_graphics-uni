 ...
 */
void ExampleScene4::updateShading(std::vector<Vertex> &mesh, const glm::mat4 &R, const glm::vec4 lightPos) const
{
	// Viewer parameters
...
bool ExampleScene4::init()
{
	...
	R = glm::rotate(glm::mat4(), rotation, glm::vec3(0.0, 1.0, 0.0)); // Rotate object around y-axis

	// Update our model's shading with some orientation in the world
	glm::vec4 lightPos(2.0f, 4.0f, 3.0f, 1.0f);
	updateShading(sphere, R, lightPos);
...
//	updateShading(sphere, modelMat, lightPos);
	...
	glm::vec4 lightInViewPos = viewMat * R * lightPos; // Light tied to object transformation

//	std::cout << "Light in view pos: " << lightInViewPos.x << ", " << lightInViewPos.y << ", " << lightInViewPos.z << std::endl;

	glm::mat4 modelViewMat = viewMat * R;
	/*
