...
#include "objparser.h"
#include "../Mycode/include/MonkeyScene.h"
#include "../Mycode/include/Assignment2.h"
#include "../Mycode/include/Assignment3.h"
		...
		for (vertex_group_it = group.begin(); vertex_group_it != group.end(); ++vertex_group_it)
		{
			...
			for (material_it = vertex_group_it->second.begin(); material_it != vertex_group_it->second.end(); ++material_it)
			{
				...
				for (face_range_it = material_it->second.begin(); face_range_it != material_it->second.end(); ++face_range_it)
				{
					...
					glm::vec4 position = vbuffer.pos[vbuffer.indices[face_range_it->startIndex]];
					std::cout << "     First vertex position: " << position.x << ", " << position.y << ", " << position.z << std::endl;
				}
			}
		}
	}
